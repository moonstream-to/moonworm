...
class FunctionCallCrawlerState(ABC):
    ...
    @abstractmethod
    def process_function_call(self, function_call: ContractFunctionCall):
        """
...
class EthereumStateProviderW3Impl(EthereumStateProvider):
    """
...
class FunctionCallCrawler:
    ...
    def process_transaction(self, transaction: Dict[str, Any]):
        ...
        print("here")
        self.state.process_function_call(function_call)

    def crawl(self, from_block: int, to_block: int):
        for block_number in range(from_block, to_block + 1):
            print(f"Crawling block {block_number}")
...
class MockupState(FunctionCallCrawlerState):
    """
    Mockup state for function call crawler.
    """

    def __init__(self):
        self.last_crawled_block = 0

    def get_last_crawled_block(self):
        return self.last_crawled_block

    def process_function_call(self, function_call: ContractFunctionCall):
        print(f"Got a function call: {function_call}")
        self.last_crawled_block = function_call.block_number


crawler = FunctionCallCrawler(
    state=MockupState(),
    ethereum_state_provider=EthereumStateProviderW3Impl(
